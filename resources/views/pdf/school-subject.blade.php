...
<body>
    ...
    <table>
        ...
        <tbody>
            @forelse ($data as $school_subject)
                <tr>
                    <td style="text-align: center">{{ $loop->iteration }}</td>
                    <td>{{ $school_subject->code ?? '-' }}</td>
                    <td>{{ $school_subject->name ?? '-' }}</td>
                    <td>{{ $school_subject->status ? 'Aktif' : 'Non Aktif' }}</td>
                    <td>{{ $school_subject->teacher->name ?? '-' }}</td>
                    <td>{{ ucwords($school_subject->teacher->status ?? '-') }}</td>
                </tr>
            @empty
                <tr>
